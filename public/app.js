...
function filterKatalog() {
    ...
    listEl.innerHTML = filtered.map(i => {
        ...
        return `<div class="bg-white border border-slate-200 p-2.5 rounded-xl cursor-pointer hover:border-blue-400 active:scale-95 transition-all shadow-sm flex flex-col justify-between" onclick="tambahItem('${safeAttr(i.barcode)}')">
          <div><div class="font-bold text-[11px] uppercase truncate leading-tight mb-1" title="${safeStr(i.nama)}">${safeStr(i.nama)}</div><div class="text-[9px] font-mono text-slate-400 mb-1.5">${safeStr(i.barcode)}</div><div class="text-blue-600 font-black text-sm">${formatRupiah(i.harga_jual)}</div></div>
...
async function prosesBayar() {
    ...
    try {
        ...
        lastTrxData = { no, total, bayar: b, kembali: b - total, items: [...keranjang], metode: metodeBayarActive, kasir };
