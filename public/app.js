...
async function loadBarang(){
    try {
        ...
        arr.forEach(i => {
            const b = safeStr(i.barcode); const n = safeStr(i.nama); const k = safeStr(i.kategori || '-'); const st = safeStr(i.satuan || 'pcs');
            const hj = i.harga_jual || 0; const hb = i.harga_beli || 0; const s = i.stok || 0;
            const stokLabel = s <= 5 ? `<span class="bg-red-50 text-red-600 px-1.5 py-0.5 rounded font-black animate-pulse border border-red-200">${s} ${st}</span>` : `<span class="font-black">${s} <span class="text-[9px] text-slate-400 font-bold">${st}</span></span>`;
            html += `<tr class="border-b hover:bg-slate-50 transition"><td class="p-3"><div class="font-bold text-slate-800">${n} <span class="bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ml-1 border">${k}</span></div><div class="text-[9px] text-slate-400 font-mono mt-0.5">${b}</div></td><td class="p-3 text-right font-bold text-slate-500">${formatRupiah(hb)}</td><td class="p-3 text-right font-black text-blue-600">${formatRupiah(hj)}</td><td class="p-3 text-center">${stokLabel}</td><td class="p-3 text-center"><button onclick="siapkanEdit('${b}','${n.replace(/'/g,"\\'")}','${hj}','${s}','${hb}','${k.replace(/'/g,"\\'")}','${st.replace(/'/g,"\\'")}')" class="text-blue-600 font-bold hover:underline mr-3 text-[10px] uppercase">Edit</button><button onclick="hapusBarang('${b}')" class="text-red-500 font-bold hover:underline text-[10px] uppercase">Hapus</button></td></tr>`;
        });
...
async function simpanBarang() {
    ...
    const hb = document.getElementById('addHargaBeli').value || 0; const s = document.getElementById('addStok').value || 0; const k = document.getElementById('addKategori').value.trim() || '-'; const st = document.getElementById('addSatuan').value.trim() || 'pcs';

    if(!b || !n || !hj) return showAlert('Perhatian', 'Kolom Barcode, Nama Produk, dan Harga Jual WAJIB diisi!', 'error');
    ...
    try {
        const res = await fetch(isEditMode ? `/api/produk/${d.barcode}` : '/api/produk', { method: isEditMode ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(d) }); 
        ...
        if(res.ok && respData.success) {
            ...
            showAlert('Gagal', respData.pesan || 'Barcode sudah digunakan.', 'error');
        }
...
async function hapusBarang(b) { showConfirm('Hapus Barang?', 'Anda yakin ingin menghapus barang ini secara permanen?', async () => { await fetch(`/api/produk/${b}`, { method: 'DELETE' }); loadBarang(); loadKatalogKasir(); showAlert('Terhapus', 'Barang berhasil dihapus.', 'success'); }); }

// ================= KASIR (FIX BACA DATA) =================
async function loadKatalogKasir(){
    try {
        const res = await fetch('/api/produk'); 
        const d = await res.json(); // <-- BACA DATA SECARA BENAR (CUMA 1X)
        dataKatalog = Array.isArray(d) ? d : []; // <-- SIMPAN DATA KE VARIABEL GLOBAL
        
        var cats = ['All'];
        dataKatalog.forEach(function(i) {
            var kat = safeStr(i.kategori || '-');
            if(!cats.includes(kat)) cats.push(kat);
        });
...
function filterKatalog() {
    ...
    const filtered = dataKatalog.filter(i => { 
        const n = safeStr(i.nama).toLowerCase(); 
        const b = safeStr(i.barcode).toLowerCase(); 
        const k = safeStr(i.kategori || '-'); 
    ...
    if(filtered.length === 0) { 
        listEl.innerHTML='<div class="col-span-full py-8 text-center text-slate-400 italic text-xs font-bold">Barang tidak ditemukan</div>'; 
        return; 
    }
    
    listEl.innerHTML = filtered.map(i => {
        const stockLabel = i.stok <= 5 ? `<span class="bg-red-50 text-red-500 border border-red-200 px-1 rounded animate-pulse">Sisa: ${i.stok} ${safeStr(i.satuan)}</span>` : `<span class="bg-slate-50 text-slate-400 border px-1 rounded">Stok: ${i.stok} ${safeStr(i.satuan)}</span>`;
        return `<div class="bg-white border border-slate-200 p-2.5 rounded-xl cursor-pointer hover:border-blue-400 active:scale-95 transition-all shadow-sm flex flex-col justify-between" onclick="tambahItem('${safeStr(i.barcode)}')"><div><div class="font-bold text-[11px] uppercase truncate leading-tight mb-1" title="${safeStr(i.nama)}">${safeStr(i.nama)}</div><div class="text-[9px] font-mono text-slate-400 mb-1.5">${safeStr(i.barcode)}</div><div class="text-blue-600 font-black text-sm">${formatRupiah(i.harga_jual)}</div></div><div class="mt-2 text-[8px] uppercase font-bold tracking-widest w-fit">${stockLabel}</div></div>`
    }).join('');
}
...
function handleScan(e) {
    ...
    if(e.key === 'Enter') {
        ...
        const item = dataKatalog.find(i => safeStr(i.barcode) === bc);
...
function tambahItem(b){
    const i = dataKatalog.find(x => safeStr(x.barcode) === b); 
    if(!i || i.stok <= 0) return showAlert('Stok Habis', 'Barang ini kosong.', 'error'); 
    const x = keranjang.findIndex(k => safeStr(k.barcode) === b); 
    if(x > -1) {
...
function renderKeranjang(){
    ...
    document.getElementById('areaKeranjang').innerHTML=`<table class="w-full">` + keranjang.map((i,idx) => {
        ...
        return `<tr class="border-b hover:bg-slate-50 transition"><td class="py-2 px-1"><div class="font-bold text-[11px] leading-tight" style="word-break: break-word;" title="${safeStr(i.nama)}">${safeStr(i.nama)}</div><div class="text-[9px] text-slate-400 mt-0.5">@${formatRupiah(i.harga_jual)}</div></td><td align="center"><div class="flex items-center justify-center gap-1.5 bg-slate-100 rounded-lg p-1"><button onclick="gantiQty(${idx},-1)" class="w-6 h-6 bg-white text-red-500 rounded shadow-sm font-black">-</button><span class="w-5 font-black text-[11px] text-center">${i.qty}</span><button onclick="gantiQty(${idx},1)" class="w-6 h-6 bg-white text-blue-600 rounded shadow-sm font-black">+</button></div></td><td align="right" class="font-black text-[11px]">${formatRupiah(i.subtotal)}</td></tr>`
    }).join('') + `</table>`; 
...
async function loadDashboard() {
    try {
        ...
        let omzetHariIni = 0, labaHariIni = 0; const last7Days = []; const omzet7Days = [];
        for(let i=6; i>=0; i--) {
            ...
            if(i === 0) { omzetHariIni = dailyOmzet; labaHariIni = dailyTx.reduce((sum, t) => sum + (parseFloat(t.total_bayar||0) - parseFloat(t.total_modal||0)), 0); }
        }
        document.getElementById('dashOmzetHariIni').innerText = formatRupiah(omzetHariIni); document.getElementById('dashLabaHariIni').innerText = formatRupiah(labaHariIni);
        document.getElementById('dashTotalProduk').innerText = (Array.isArray(prData)?prData:[]).length; document.getElementById('dashTotalStok').innerText = (Array.isArray(prData)?prData:[]).reduce((sum, p) => sum + parseInt(p.stok||0), 0);

        if (typeof Chart !== 'undefined') {
            if(chartOmzetInstance) chartOmzetInstance.destroy(); const ctxOmzet = document.getElementById('chartOmzet').getContext('2d');
            chartOmzetInstance = new Chart(ctxOmzet, { type: 'line', data: { labels: last7Days, datasets: [{ label: 'Omzet (Rp)', data: omzet7Days, borderColor: '#2563eb', backgroundColor: 'rgba(37, 99, 235, 0.1)', borderWidth: 2, fill: true, tension: 0.3 }] }, options: { responsive: true, maintainAspectRatio: false } });
            if(chartTerlarisInstance) chartTerlarisInstance.destroy(); const ctxTerlaris = document.getElementById('chartTerlaris').getContext('2d'); const topData = Array.isArray(trData) ? trData : [];
            chartTerlarisInstance = new Chart(ctxTerlaris, { type: 'bar', data: { labels: topData.map(d => safeStr(d.nama_barang)), datasets: [{ label: 'Terjual (Qty)', data: topData.map(d => d.total_qty), backgroundColor: '#16a34a', borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false } });
        }
    } catch(e) { }
}
...
async function loadPengaturan(){
    try {
        ...
        if(d){ profilToko=d; document.getElementById('setNamaToko').value=d.nama_toko||''; document.getElementById('setAlamat').value=d.alamat_toko||''; document.getElementById('setTelp').value=d.telp_toko||''; document.getElementById('headerNamaToko').innerText=d.nama_toko||'POSweb by Rsby'; document.getElementById('headerAlamatTelp').innerText=`${d.alamat_toko||''} | Telp: ${d.telp_toko||''}`; document.getElementById('strukNamaTokoPrint').innerText=d.nama_toko||''; document.getElementById('strukAlamatTelpPrint').innerText=d.alamat_toko||''; }
        const kertas = localStorage.getItem('ukuranKertas') || '58mm'; const footer = localStorage.getItem('footerStruk') || 'Terima Kasih';
        document.getElementById('setUkuranKertas').value = kertas; document.getElementById('setFooterStruk').value = footer; document.getElementById('strukFooterText').innerText = footer;
...
async function simpanPengaturan(){ const d={nama_toko:document.getElementById('setNamaToko').value,alamat_toko:document.getElementById('setAlamat').value,telp_toko:document.getElementById('setTelp').value}; try { await fetch('/api/pengaturan',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)}); showAlert('Sukses', 'Profil Toko tersimpan.', 'success'); loadPengaturan(); } catch(e) { showAlert('Gagal', 'Gagal menyimpan pengaturan.', 'error'); } }
function simpanPengaturanPrinter() { const kertas = document.getElementById('setUkuranKertas').value; const footer = document.getElementById('setFooterStruk').value; localStorage.setItem('ukuranKertas', kertas); localStorage.setItem('footerStruk', footer); document.getElementById('strukFooterText').innerText = footer || 'Terima Kasih'; const areaStruk = document.getElementById('areaStruk'); if(kertas === '80mm') { areaStruk.classList.replace('max-w-[58mm]', 'max-w-[80mm]'); } else { areaStruk.classList.replace('max-w-[80mm]', 'max-w-[58mm]'); } showAlert('Sukses', 'Printer dan Struk tersimpan.', 'success'); }

// ================= FITUR LAPORAN =================
async function loadDataLaporan(){ try { const res = await fetch('/api/transaksi'); const d = await res.json(); semuaTransaksi = Array.isArray(d) ? d : []; renderLaporan(currentLaporanType || 'harian'); } catch(e) { semuaTransaksi = []; renderLaporan('harian'); } }
...
function gambarTabelLaporan(data){
    ...
    if(currentLaporanType === 'harian') {
        ...
        if(!data || data.length === 0) { list.innerHTML = `<tr><td colspan="3" class="p-6 text-center text-slate-400 italic font-bold">Data kosong.</td></tr>`; } 
            ...
            data.forEach(t => { const tb = parseFloat(t.total_bayar) || 0; const tm = parseFloat(t.total_modal) || 0; o += tb; m += tm; list.innerHTML += `<tr class="border-b hover:bg-slate-50 transition"><td class="p-4"><div class="font-bold text-blue-600 font-mono text-[10px] uppercase">${t.no_struk}</div><div class="text-[9px] text-slate-400 font-bold mt-0.5">${new Date(t.tanggal).toLocaleString('id-ID')}</div><div class="text-[8px] bg-slate-100 text-slate-500 inline-block px-1.5 py-0.5 rounded border mt-1 uppercase font-bold tracking-widest">Kasir: ${safeStr(t.kasir) || 'Admin'} • ${t.metode_bayar || 'TUNAI'}</div></td><td class="p-4 text-right"><div class="font-bold text-slate-800 text-xs">${formatRupiah(tb)}</div><div class="text-[9px] text-green-600 font-black mt-0.5">Laba: ${formatRupiah(tb-tm)}</div></td><td class="p-4 text-center"><button onclick="lihatDetailStruk('${t.id}','${t.no_struk}','${t.tanggal}',${tb},'${safeStr(t.kasir) || 'Admin'}','${t.metode_bayar || 'TUNAI'}')" class="bg-slate-100 border text-slate-600 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase hover:bg-slate-200 shadow-sm active:scale-95">Audit</button></td></tr>`; }); 
        }
    } else {
...
async function lihatDetailStruk(id, no, tgl, tot, kas, metode) { try { const res = await fetch('/api/transaksi/detail/' + id); const d = await res.json(); memoriRevisi = { id, no, items: d }; let h = `<div class='mb-4 text-center border-b pb-3'><div class='font-black text-sm uppercase text-slate-800 tracking-tight'>${profilToko.nama_toko||''}</div><div class='text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-widest'>${new Date(tgl).toLocaleString('id-ID')}</div><div class='text-[10px] bg-blue-50 text-blue-600 inline-block px-2 py-0.5 rounded font-mono font-bold mt-1 border border-blue-100'>${no}</div><div class='text-[9px] font-bold text-slate-500 mt-1 uppercase tracking-widest'>Kasir: ${kas} • ${metode}</div></div><table class='w-full text-xs mb-3'><tbody class='divide-y divide-slate-100 border-t border-b'>`; d.forEach(i => h += `<tr><td class='py-2.5'><div class='font-bold text-slate-700'>${safeStr(i.nama_barang)}</div><div class='text-[9px] text-slate-500 font-medium'>@${formatRupiah(i.harga)}</div></td><td class='py-2.5 text-center text-[10px] font-bold text-blue-600'>x${i.qty}</td><td class='py-2.5 text-right font-black text-slate-800'>${formatRupiah(i.subtotal)}</td></tr>`); h += `</tbody></table><div class='flex justify-between font-black text-sm pt-1 text-slate-800'><span>TOTAL</span><span class='text-blue-600'>${formatRupiah(tot)}</span></div>`; const reprintData = { no: no, tanggal: new Date(tgl).toLocaleString('id-ID'), kasir: kas, metode: metode, total: tot, items: d }; const reprintBtnHTML = `<button onclick='cetakUlangStruk(${JSON.stringify(reprintData)})' class="bg-blue-500 text-white font-bold py-3 text-sm rounded-xl shadow-md hover:bg-blue-600 transition uppercase tracking-widest active:scale-95 mb-2 w-full">🖨️ Print Ulang Struk</button>`; document.getElementById('isiModalStruk').innerHTML = h; document.querySelector('#modalStrukLaporan .flex-col.gap-2').innerHTML = reprintBtnHTML + `<button onclick="revisiTransaksi()" class="bg-orange-500 text-white font-bold py-3 text-sm rounded-xl shadow-md hover:bg-orange-600 transition uppercase tracking-widest active:scale-95">Hapus & Void (Retur)</button><button onclick="tutupModalStruk()" class="bg-slate-100 text-slate-600 font-bold py-3 text-sm rounded-xl hover:bg-slate-200 transition">Tutup Dialog</button>`; document.getElementById('modalStrukLaporan').classList.remove('hidden'); } catch(e) { showAlert('Error', 'Gagal memuat rincian.', 'error'); } }
function cetakUlangStruk(data) { document.getElementById('strukTanggal').innerText = data.tanggal; document.getElementById('strukNomor').innerText = data.no + " (REPRINT)"; document.getElementById('strukNamaKasir').innerText = data.kasir.toUpperCase(); document.getElementById('strukItem').innerHTML = data.items.map(i=>`<tr><td class="pb-1">${safeStr(i.nama_barang)}</td><td align="right" class="pb-1 font-bold">${i.qty}</td></tr>`).join(''); document.getElementById('strukTotalPrint').innerText = formatRupiah(data.total); document.getElementById('strukMetodePrint').innerText = data.metode.toUpperCase(); if(data.metode.toUpperCase() === 'QRIS') { document.getElementById('areaStrukTunai').classList.add('hidden'); document.getElementById('areaStrukKembali').classList.add('hidden'); } else { document.getElementById('areaStrukTunai').classList.remove('hidden'); document.getElementById('areaStrukKembali').classList.remove('hidden'); document.getElementById('strukTunaiPrint').innerText = formatRupiah(data.total); document.getElementById('strukKembaliPrint').innerText = "Rp0"; } doPrintAction(document.getElementById('areaStruk'), () => {}); }
